...
export async function getFolderToRunCommandIn(logger: Logger, placeHolder: string, selection?: vs.Uri): Promise<string | undefined> {
  ...
  if (!selectableFolders || !selectableFolders.length) {
    vs.window.showWarningMessage(`No hetu projects were found.`);
    return undefined;
  }
