...
import { Sdks, Logger } from "../../shared/interfaces";
...
export function getAnalyzerArgs(logger: Logger) {
  const analyzerPath = config.analyzerPath || analyzerSnapshotPath;
  ...
  return buildAnalyzerArgs(analyzerPath);
}

function buildAnalyzerArgs(analyzerPath: string) {
  let analyzerArgs = [];
